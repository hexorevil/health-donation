      ...
      <div style={{ flex: 1, maxWidth: "88%" }}>
        {isUser ? (
          ...
          <div style={{ background: "var(--bg-card)", border: `1px solid ${modeColor}33`, borderRadius: "12px 4px 12px 12px", padding: "0.8rem 1.1rem" }}>
            ...
            <div className="prose">
              <ReactMarkdown remarkPlugins={[remarkGfm]} components={{ pre: ({ children, ...props }) => ( <div style={{ position: "relative" }}> <pre {...props}>{children}</pre> <CopyButton text={String((children as React.ReactElement)?.props?.children ?? "")} /> </div> ) }}>
                {msg.content || "▍"}
              </ReactMarkdown>
            </div>
          </div>
